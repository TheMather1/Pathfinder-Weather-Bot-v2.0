...
object Interface{
    ...
    private fun Properties.manip(returnIfValid: Boolean) {
        fun hasImportant(): Boolean = keys.containsAll(listOf("climate", "elevation", "webhook_url", "forecast_url"))
        ...
        fun add(){
            ...
            fun webhook(){
                ...
                while (readLine()?.trim()?.let { setProperty("webhook_url", it); false } ?: println("No input detected. Try again.").let { true }) Unit
            }
            fun forecast(){
                println("Enter forecast url:")
                while (readLine()?.trim()?.let { setProperty("forecast_url", it); false } ?: println("No input detected. Try again.").let { true }) Unit
            }
            while (true){
                println("\nEnter property to change, or DONE to return to the previous menu. Valid properties are CLIMATE, ELEVATION, DESERT, WEBHOOK_URL and FORECAST_URL.")
                when(input){
                    "CLIMATE" -> climate()
                    "ELEVATION" -> elevation()
                    "DESERT" -> desert()
        ...
        while (true) {
            ...
            if (!keys.contains("forecast_url")) println("Forecast URL not configured.")
